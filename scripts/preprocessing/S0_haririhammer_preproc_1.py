...
if not os.path.isdir(subj_preprocpath):
    os.mkdir(subj_preprocpath)

if not os.path.isdir(pathjoin(subj_preprocpath, "haririhammer")):
    os.mkdir(pathjoin(subj_preprocpath, "haririhammer"))
    os.mkdir(pathjoin(subj_preprocpath, "haririhammer", "rejection_ica"))
...
rawdata = mne.io.read_raw_ctf(
    subj + "_ses-01_task-haririhammer_run-01_meg.ds", preload=True
)
...
rejectlog_filename = pathjoin(outputpath, "reject_log_continuous.npz")
if os.path.isfile(rejectlog_filename):
    ...
    reject_log.save(pathjoin(outputpath, "reject_log_continuous.npz")) # save rejection log (must end with .npz)
    toc = time() - tic
...
if np.any(reject_log.bad_epochs):
    ...
    figure.savefig(pathjoin(outputpath, "autoreject_beforeICA_continuous.jpg"))

figure = reject_log.plot("horizontal")
figure.savefig(pathjoin(outputpath, "rejectlog_beforeICA_continuous.jpg"))
...
plt.savefig(pathjoin(outputpath, "ICA_components_continuous.jpg"))
plt.close()

ica.plot_sources(rawdata, show_scrollbars=False)
plt.savefig(pathjoin(outputpath, "ICA_sources_continuous.jpg"))
plt.close()

ica.plot_properties(rawdata, picks=np.arange(20))
plt.savefig(pathjoin(outputpath, "ICA_properties_continuous.jpg"))
plt.close()

ica.save(pathjoin(outputpath, subj + "_haririhammer_continuous-ica.fif"))
