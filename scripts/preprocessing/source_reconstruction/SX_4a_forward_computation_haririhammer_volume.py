...
fig.savefig(pathjoin(subj_preprocpath, "haririhammer", "bem.png"))

# Set the transformation matrix path
trans = pathjoin(subj_preprocpath, "haririhammer", "alignment-trans.fif")

# Setup source space
surface = pathjoin(subjects_dir, i_subj, "bem", "inner_skull.surf")
vol_src = mne.setup_volume_source_space(
    i_subj, subjects_dir=subjects_dir, surface=surface
) 
print(vol_src)

# Visualize BEM with source space
fig = mne.viz.plot_bem(src=vol_src, **plot_bem_kwargs)
fig.savefig(pathjoin(subj_preprocpath, "haririhammer", "bem_vol_src.png"))
...
print(f"Before: {vol_src}")
print(f'After:  {fwd["src"]}')

leadfield = fwd["sol"]["data"]
print(f"Leadfield size : {leadfield.shape[0]} sensors x {leadfield.shape[1]} dipoles")

mne.write_forward_solution(pathjoin(subj_preprocpath, "haririhammer", "forward_volume-fwd.fif"), fwd, overwrite=True)
print(f"Forward solution saved to {pathjoin(subj_preprocpath, 'haririhammer', 'forward_volume-fwd.fif')}")