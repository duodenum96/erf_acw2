...
from haririhammer_badICs import exclude, badics
...
ica_filename = pathjoin(outputpath, subj + "_haririhammer_continuous-ica.fif")


ica = pp.read_ica(ica_filename)
...
rejectlog_name = pathjoin(outputpath, "reject_log_round2_cont.npz")

reject_log.save(rejectlog_name, overwrite=True)

filename = pathjoin(outputpath, subj + "_haririhammer_preprocessed_cont-epo.fif.gz")

epochs_ar2.save(
    filename, fmt="double", overwrite=True
)
