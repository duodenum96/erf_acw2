...
pass_or_fail_itpc = [] # is there a significance after violin plots
multcomps_itpc = []
for i in range(len(Xs_itpc)):
    #######################################
    # plot
    #######################################

    loadname = f"/BICNAS2/ycatal/erf_acw2/results/erf/haririhammer/{taskname}_erp_permutationtest_st_emo_" \
               f"{tests[i]}_induced_itpc_p_0d001.pkl"
    ...
    evokeds = {cond: tasks_itpc[cond] for cond in comparisons[i]}
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        for k in range(len(ax_signals)):
            ims = ax_signals[k].imshow(gas[keys[k]],
                                 origin="lower",
        ...
        multcomps_itpc.append(multcomp)
        stattexts = []
        ...
        if len(multcomp) == 3:
            pass_or_fail_itpc.append(np.any(multcomp["p-corr"].to_numpy() < 0.05))
        else:
            pass_or_fail_itpc.append(np.any(multcomp["p-unc"].to_numpy() < 0.05))
        ...
        if pass_or_fail_itpc[-1]:
            fig.savefig(pathjoin(figpath, "induced", f"{taskname}_{i}_cluster{i_clu}_face_vs_shape_X_induced_itpc.png")) #, dpi=300, transparent=False)
        else:
            fig.savefig(pathjoin(figpath, "induced", f"{taskname}_{i}_cluster{i_clu}_face_vs_shape_induced_itpc.png")) #, dpi=300, transparent=False)
