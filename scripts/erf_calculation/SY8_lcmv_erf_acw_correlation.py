...
significant_stuff = poststim_erfs[sig_indices[0], sig_indices[1], sig_indices[2], :]
poststim_average = np.sqrt(significant_stuff**2).mean(axis=1)
sig_ints = ints[sig_indices[0], sig_indices[2]]

pg.corr(sig_ints, poststim_average)
...
for i, i_roi in enumerate(sig_indices[2]):
    roi_ints[ctx_labels[i_roi]][tasknames[sig_indices[1][i]]].append(ints[sig_indices[0][i], i_roi])
    roi_erfs[ctx_labels[i_roi]][tasknames[sig_indices[1][i]]].append(
        np.mean(
            np.sqrt(poststim_erfs[sig_indices[0][i], sig_indices[1][i], i_roi, :] ** 2)
        )
    )
    roi_subjs[ctx_labels[i_roi]][tasknames[sig_indices[1][i]]].append(sig_indices[0][i])
...
model = bmb.Model("ints_z ~ erfs_z + (erfs_z | labels) + (erfs_z | tasknames)", df, family="t")
results = model.fit()
summary = az.summary(results)
summary.to_csv("anan.csv")
