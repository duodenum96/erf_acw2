...
n_freqbands = len(selected_freqbands)

nrow = 2
ncol =  3

rhos = np.zeros((nrow,ncol,3, n_freqbands)) # row x col x happy/sad/shape
pvals = np.zeros((nrow,ncol,3, n_freqbands))
...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        for m, m_key in enumerate(erf_violins.keys()):
            for l, l_freq in enumerate(selected_freqbands):
                corr_results = sp.stats.spearmanr(np.nanmean(induced_data[m_key][l_freq][:, picks], axis=0), np.nanmean(restmean[picks, :], axis=1))
                rhos[row, col, m, l] = corr_results.statistic
                pvals[row, col, m, l] = corr_results.pvalue
