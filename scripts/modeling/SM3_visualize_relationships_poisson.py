...
f = h5py.File(
    "/BICNAS2/ycatal/erf_acw2/scripts/modeling/results/sensitivity_control_pythonic_poisson.jld2", 
    "r",
)
...
for i_area, area in enumerate(areas):
    for i_param, param in enumerate(params):
        for i_measure, measure in enumerate(measures):
            ...
            ax[row_idx, i_param].grid(False)

plt.suptitle("Poisson distributed events")
f.savefig(join(figpath, "supplementary_sensitivity_control_poisson.png"), dpi=300, transparent=True)
