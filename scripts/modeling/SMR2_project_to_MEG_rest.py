...
# nohup python SMR2_project_to_MEG_rest.py > log/SMR2_project_to_MEG_rest.log 2>&1 &
# echo $! > log/SMR2_project_to_MEG_rest.pid
import mne
import numpy as np
import h5py
import os
...
for dataset_idx, (dataset_name, dataset) in enumerate(zip(dataset_names, datasets)):
    ...
    for j_sim in range(nsim):
        ...
        for k_parameter in range(ngamma):
            print(f"  Parameter {k_parameter+1}/{ngamma}")
            ...
            raw = mne.simulation.simulate_raw(info, source_simulator, forward=fwd, n_jobs=20)
            ...
            acw_results[dataset_name][:, k_parameter, j_sim] = acws
