...
theta = powers["theta"] # (channels, subjects)
alpha = powers["alpha"]
beta1 = powers["beta1"]
beta2 = powers["beta2"]
gamma = powers["gamma"]
        ...
        for k, k_comp in enumerate(comparisons[i]):
            for m in range(nsubj):
                for i_channel, channel in enumerate(picks):
                    ...
                    alphas.append(alpha[channel, m])
                    beta1s.append(beta1[channel, m])
                    beta2s.append(beta2[channel, m])
                    gammas.append(gamma[channel, m])
                    thetas.append(theta[channel, m])
...
data.to_csv("/BICNAS2/ycatal/erf_acw2/results/data_st_and_oscillations_2fit.csv")
