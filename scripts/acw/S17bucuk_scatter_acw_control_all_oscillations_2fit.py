...
powers = pklload(f"/BICNAS2/ycatal/erf_acw2/results/int_fooof/rest_all_powers_2fit.pkl")
...
for i in range(len(Xs)):
    for j in range(len(comparisons[i])):
        mvar_tables_osc[comparisons[i][j]] = []
...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        for m, m_key in enumerate(erf_violins.keys()):
            ...
            X = np.array(
                (
                    np.nanmean(theta[picks, :], axis=1), 
                    np.nanmean(alpha[picks, :], axis=1), 
                    np.nanmean(beta1[picks, :], axis=1), 
                    np.nanmean(beta2[picks, :], axis=1), 
                    np.nanmean(gamma[picks, :], axis=1), 
                    np.nanmean(restmean[picks, :], axis=1)
                )
            ).T
            mv_regression_osc = pg.linear_regression(z_score(X, axis=0), z_score(y, axis=0), relimp=True)

            mvar_tables_osc[comparisons[i][m]].append(mv_regression_osc)
...
def create_regression_summary_table(mvar_tables):
    ...
    predictor_names = {'x1': 'Theta', 'x2': 'Alpha', 'x3': 'Beta1', 'x4': 'Beta2', 'x5': 'Gamma', 'x6': 'ACW'}
        ...
        for cluster_idx, regression_df in enumerate(regression_list):
            ...
            for _, row in predictors.iterrows():
                ...
                summary_data.append({
                    ...
                    'Relative Importance': f"{row['relimp']:.3f} ({row['relimp_perc']:.3f}%)"
                })
...
regression_summary = create_regression_summary_table(mvar_tables_osc)

# Apply p-value correction per cluster and condition
regression_summary["p-value (corrected)"] = np.nan

# Group by condition and cluster to correct p-values separately
for condition in regression_summary['Condition'].unique():
    for cluster in regression_summary['Cluster'].unique():
        # Get mask for current condition and cluster
        mask = (regression_summary['Condition'] == condition) & (regression_summary['Cluster'] == cluster)
        
        if mask.sum() > 0:  # Check if there are any rows for this combination
            # Extract p-values for this group
            ps_group = regression_summary.loc[mask, "P-value"].values
            
            # Apply FDR correction within this group
            ps_corrected_group = pg.multicomp(ps_group, method="fdr_bh")[1]
            
            # Assign corrected p-values back to the dataframe
            regression_summary.loc[mask, "p-value (corrected)"] = ps_corrected_group

regression_summary[regression_summary["Predictor"] == "ACW"]["p-value (corrected)"] < 0.05
regression_summary.iloc[[77, 89], :]

regression_summary.to_csv(pathjoin(figpath, "f5_acw_control_all_oscillations_regression_summary_2fit.csv"), index=False)

# Non Significant ones:
# Probe Face Happy        Cluster 1       ACW
# Probe Face Sad        cluster 1       ACW 