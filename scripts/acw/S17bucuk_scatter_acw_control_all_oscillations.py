...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        for m, m_key in enumerate(erf_violins.keys()):
            ...
            X = np.array((np.nanmean(theta[picks, :], axis=1), np.nanmean(alpha[picks, :], axis=1), np.nanmean(beta[picks, :], axis=1), np.nanmean(restmean[picks, :], axis=1))).T
...
def create_regression_summary_table(mvar_tables):
    ...
    predictor_names = {'x1': 'Theta', 'x2': 'Alpha', 'x3': 'Beta', 'x4': 'ACW'}
...
regression_summary.to_csv(pathjoin(figpath, "f5_acw_control_all_oscillations_regression_summary.csv"), index=False)
