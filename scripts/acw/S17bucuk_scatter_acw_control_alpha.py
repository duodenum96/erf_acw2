...
powers = pklload(f"/BICNAS2/ycatal/erf_acw2/results/int_fooof/rest_all_powers.pkl")

theta = powers["theta"]
alpha = powers["alpha"]

restmean = np.nanmedian(hh_acw, axis=0) 
...
for i in range(len(Xs)):
    for j in range(len(comparisons[i])):
        mvar_tables_alpha[comparisons[i][j]] = []
        mvar_tables_theta[comparisons[i][j]] = []
...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        for m, m_key in enumerate(erf_violins.keys()):
            ...
            mv_regression_alpha = pg.linear_regression(z_score(X, axis=0), z_score(y, axis=0))

            mvar_tables_alpha[comparisons[i][m]].append(mv_regression_alpha)

            y = np.nanmean(np.array(erf_violins[m_key]), axis=0)
            X = np.array((np.nanmean(theta[picks, :], axis=1), np.nanmean(restmean[picks, :], axis=1))).T
            mv_regression_theta = pg.linear_regression(z_score(X, axis=0), z_score(y, axis=0))

            mvar_tables_theta[comparisons[i][m]].append(mv_regression_theta)
...
def create_regression_summary_table(mvar_tables, alpha_or_theta):
    ...
    predictor_names = {'x1': alpha_or_theta, 'x2': 'ACW'}
...
regression_summary_alpha = create_regression_summary_table(mvar_tables_alpha, "Alpha Power")

ps_alpha = regression_summary_alpha[regression_summary_alpha["Predictor"] == "Alpha Power"]["P-value"].values
ps_acw = regression_summary_alpha[regression_summary_alpha["Predictor"] == "ACW"]["P-value"].values
ps = regression_summary_alpha["P-value"].values
ps_corrected = pg.multicomp(ps, method="fdr_bh")[1]

regression_summary_alpha["P-value (corrected)"] = ps_corrected

regression_summary_alpha.to_csv(pathjoin(figpath, "f5_acw_control_alpha_regression_summary.csv"), index=False)

regression_summary_theta = create_regression_summary_table(mvar_tables_theta, "Theta Power")

ps_theta = regression_summary_theta[regression_summary_theta["Predictor"] == "Theta Power"]["P-value"].values
ps_acw = regression_summary_theta[regression_summary_theta["Predictor"] == "ACW"]["P-value"].values
ps = regression_summary_theta["P-value"].values
ps_corrected = pg.multicomp(ps, method="fdr_bh")[1]

regression_summary_theta["P-value (corrected)"] = ps_corrected

regression_summary_theta.to_csv(pathjoin(figpath, "f5_acw_control_theta_regression_summary.csv"), index=False)