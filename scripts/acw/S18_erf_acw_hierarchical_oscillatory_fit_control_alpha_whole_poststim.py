...
# nohup python S18_erf_acw_hierarchical_oscillatory_fit_control_alpha_whole_poststim.py > log/hierarchical_nuts_erf_acw_oscillatory_fit_control_alpha_whole_poststim.log &
# echo $! > log/pid_hierarchical_nuts_erf_acw_oscillatory_fit_control_alpha_whole_poststim.txt
import os
import numpy as np
import pandas as pd
import pymc as pm
...
data2["Alpha"] = zscore(data2["alpha_powers"])
data3 = data2.copy()
...
model_formula = (
    "ACW ~ 1 + ERF + Alpha + (1|Cluster) + (1|Channel) + (1|Trial)"
    " + (ERF|Cluster) + (ERF|Channel) + (ERF|Trial) + (Alpha|Channel)"
)
...
idata.to_netcdf(join(resultpath, "idata_acw_erf_oscillatory_fit_control_alpha_whole_poststim.nc"))

summary = az.summary(idata)
summary

bad_rhats = summary[summary["r_hat"] > 1.01]
print(bad_rhats)  # no bad rhats

ess_bulk = summary["ess_bulk"]
print(ess_bulk[ess_bulk < 300])  # no bad ess
