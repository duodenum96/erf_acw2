...
data = pd.read_csv("/BICNAS2/ycatal/erf_acw2/results/data_st_and_oscillations.csv")
...
model_formula = (
    "ACW ~ 1 + mERF + Alpha + Beta + Theta + (1|Cluster) + (1|Channel) + (1|Trial)"
    " + (mERF|Cluster) + (mERF|Channel) + (mERF|Trial) + (Alpha|Channel) + (Beta|Channel) + (Theta|Channel)"
)
...
model_priors = {
    ...
    "Beta": bmb.Prior("Normal", mu=0, sigma=1),
    ...
    "Beta|Channel": bmb.Prior("Normal", mu=0, sigma=bmb.Prior("Exponential", lam=1)),
...
results = az.from_netcdf(join(resultpath, "idata_acw_erf_control_all_oscillations.nc"))
...
results.posterior["Beta + Beta|Channel"] = (
    results.posterior["Beta"] + results.posterior["Beta|Channel"]
)
...
summary.to_csv(join(figpath_supp, "summary_acw_erf_control_all_oscillations.csv"))
...
f.savefig(join(figpath_supp, "prior_and_posterior_acw_erf_control_all_oscillations.png"), dpi=300)
...
plt.savefig(join(figpath_supp, "trace_acw_erf_control_all_oscillations.png"), dpi=300)
...
ax.set_title("ACW ~ mERF + mERF|Trial + mERF|Cluster + mERF|Channel + Alpha + Alpha|Channel + Beta + Beta|Channel + Theta + Theta|Channel")
# plt.yticks(fontsize=24)
plt.xticks(fontsize=24)
ax.axvline(0, color="black", linestyle="--")
f.savefig(join(figpath_supp, "forest_acw_erf_control_all_oscillations.png"), dpi=300)
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="mERF|Channel",
    figpath=join(figpath, "channels_erf_acw_control_all_oscillations.jpg"),
    mask=mask_erf,
)
...
plot_channel_effects(
    ...
    figpath=join(figpath, "channels_erf_acw_plus_control_all_oscillations.jpg"),
    mask=mask_erf_plus,
)
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="Theta|Channel",
    figpath=join(figpath, "channels_erf_acw_control_all_oscillations_theta.jpg"),
    mask=mask_theta,
)
...
plot_channel_effects(
    ...
    figpath=join(figpath, "channels_erf_acw_plus_control_all_oscillations_theta.jpg"),
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="Alpha|Channel",
    figpath=join(figpath, "channels_erf_acw_control_all_oscillations_alpha.jpg"),
    mask=mask_alpha,
)
...
plot_channel_effects(
    ...
    figpath=join(figpath, "channels_erf_acw_plus_control_all_oscillations_alpha.jpg"),
...
mask_beta = create_channel_mask(summary, "Beta|Channel")
data_topo = results.posterior["Beta|Channel"].mean(dim=["chain", "draw"]).values
hdi_topo = az.hdi(results, var_names="Beta|Channel", hdi_prob=0.94)[
    "Beta|Channel"
].values
vlims = (np.min(hdi_topo), np.max(hdi_topo))
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="Beta|Channel",
    figpath=join(figpath, "channels_erf_acw_control_all_oscillations_beta.jpg"),
    mask=mask_beta,
)

mask_beta_plus = create_channel_mask(summary, "Beta + Beta|Channel")
data_topo = results.posterior["Beta + Beta|Channel"].mean(dim=["chain", "draw"]).values
hdi_topo = az.hdi(results, var_names="Beta + Beta|Channel", hdi_prob=0.94)[
    "Beta + Beta|Channel"
].values
vlims = (np.min(hdi_topo), np.max(hdi_topo))
...
plot_channel_effects(
    ...
    chan_order=chan_order,
    vlim=vlims,
    title="Beta + Beta|Channel",
    figpath=join(figpath, "channels_erf_acw_plus_control_all_oscillations_beta.jpg"),
    mask=mask_beta_plus,
)
