...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        time_inds = template_bad.times > 0.0
...
plt.tight_layout()
fig.savefig(pathjoin(figpath, "f5_acw_whole_poststim.png"), dpi=300, transparent=False)
...
regression_summary.to_csv(pathjoin(figpath, "f5_acw_oscillatory_fit_control_alpha_regression_summary_whole_poststim.csv"), index=False)