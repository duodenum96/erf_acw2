...
data = pd.read_csv(
    "/BICNAS2/ycatal/erf_acw2/results/data_st_oscillatory_fit_whole_poststim.csv"
)
data = data.rename(
    columns={
        "erfs": "mERF",
...
data2["Alpha"] = zscore(data2["Alpha"])
data3 = data2.copy()
...
model_formula = (
    "ACW ~ 1 + mERF + Alpha + (1|Cluster) + (1|Channel) + (1|Trial)"
    " + (mERF|Cluster) + (mERF|Channel) + (mERF|Trial) + (Alpha|Channel)"
)
...
results = az.from_netcdf(
    join(resultpath, "idata_acw_erf_oscillatory_fit_control_alpha_whole_poststim.nc")
)
...
summary.to_csv(join(figpath_supp, "summary_acw_erf_oscillatory_fit_control_alpha_whole_poststim.csv"))
...
f.savefig(
    join(
        figpath_supp, "prior_and_posterior_acw_erf_oscillatory_fit_control_alpha_whole_poststim.png"
    ),
    dpi=300,
)
...
plt.savefig(
    join(figpath_supp, "trace_acw_erf_oscillatory_fit_control_alpha_whole_poststim.png"), dpi=300
)
...
ax.set_title("ACW ~ mERF + mERF|Trial + mERF|Cluster + mERF|Channel + Alpha + Alpha|Channel")
# plt.yticks(fontsize=24)
plt.xticks(fontsize=24)
ax.axvline(0, color="black", linestyle="--")
f.savefig(
    join(figpath_supp, "forest_acw_erf_oscillatory_fit_control_alpha_whole_poststim.png"), dpi=300
)
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="mERF|Channel",
    figpath=join(figpath, "channels_erf_acw_oscillatory_fit_control_alpha_whole_poststim.jpg"),
    mask=mask_erf,
)
...
plot_channel_effects(
    ...
    figpath=join(figpath, "channels_erf_acw_plus_oscillatory_fit_control_alpha_whole_poststim.jpg"),
    mask=mask_erf_plus,
)
