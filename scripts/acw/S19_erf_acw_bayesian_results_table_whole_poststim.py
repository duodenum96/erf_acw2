...
data = pd.read_csv(
    "/BICNAS2/ycatal/erf_acw2/results/data_st_whole_poststim.csv"
)
data = data.rename(
    columns={
        "erfs": "mERF",
...
model_formula = (
    "ACW ~ 1 + mERF + (1|Channel) + (1|Trial)"
    " + (mERF|Channel) + (mERF|Trial)"
)
...
summary.to_csv(join(figpath_supp, "summary_acw_erf_whole_poststim.csv"))
...
f.savefig(
    join(
        figpath_supp, "prior_and_posterior_acw_erf_whole_poststim.png"
    ),
    dpi=300,
)
...
plt.savefig(
    join(figpath_supp, "trace_acw_erf_whole_poststim.png"), dpi=300
)
...
ax.set_title("ACW ~ mERF + mERF|Trial + mERF|Channel")
# plt.yticks(fontsize=24)
plt.xticks(fontsize=24)
ax.axvline(0, color="black", linestyle="--")
f.savefig(
    join(figpath_supp, "forest_acw_erf_whole_poststim.png"), dpi=300
)
...
plot_channel_effects(
    ...
    vlim=vlims,
    title="mERF|Channel",
    figpath=join(figpath, "channels_erf_acw_whole_poststim.jpg"),
    mask=mask_erf,
)
...
plot_channel_effects(
    ...
    figpath=join(figpath, "channels_erf_acw_plus_whole_poststim.jpg"),
    mask=mask_erf_plus,
)
