...
# nohup python S18_erf_acw_hierarchical_control_all_oscillations.py > log/hierarchical_nuts_erf_acw_control_all_oscillations.log &
# echo $! > log/pid_hierarchical_nuts_erf_acw_control_all_oscillations.txt
import os
import numpy as np
import pandas as pd
import pymc as pm
...
model_formula = (
    "ACW ~ 1 + ERF + Alpha + Beta + Theta + (1|Cluster) + (1|Channel) + (1|Trial)"
    " + (ERF|Cluster) + (ERF|Channel) + (ERF|Trial) + (Alpha|Channel) + (Beta|Channel) + (Theta|Channel)"
)
...
idata.to_netcdf(join(resultpath, "idata_acw_erf_control_all_oscillations.nc"))
