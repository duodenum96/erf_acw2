...
hh_acw = pklload(f"/BICNAS2/ycatal/erf_acw2/results/int/rest_int.pkl")["rest_acw_50s"]

restmean = np.nanmean(hh_acw, axis=0)
...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        idxs_dict["i"].append(i)
        idxs_dict["i_clu"].append(i_clu)
...
all_ch_inds = []
all_time_inds = []
for i, i_i in enumerate(idxs_dict["i"]):
    i_clu = idxs_dict["i_clu"][i]
    loadname = f"/BICNAS2/ycatal/erf_acw2/results/erf/{taskname}/{taskname}_erp_permutationtest_st_emo_{tests[i_i]}.pkl"
    cluster_results = pklload(loadname)
    
    p_accept = 0.001
    good_cluster_inds = np.where(cluster_results["cluster_p"] < p_accept)[0]

    clu_idx = good_cluster_inds[i_clu]

    time_inds, space_inds = np.squeeze(cluster_results["clusters"][clu_idx])
    ch_inds = np.unique(space_inds)
    time_inds = np.unique(time_inds)

    all_ch_inds.append(ch_inds)
    all_time_inds.append(time_inds)
    
cluster1_ch = np.union1d(all_ch_inds[0], all_ch_inds[1])
cluster2_ch = np.union1d(all_ch_inds[2], all_ch_inds[3])
cluster3_ch = np.union1d(all_ch_inds[4], all_ch_inds[5])

cluster1_time = np.union1d(all_time_inds[0], all_time_inds[1])
cluster2_time = np.union1d(all_time_inds[2], all_time_inds[3])
cluster3_time = np.union1d(all_time_inds[4], all_time_inds[5])

channel_clusters = [cluster1_ch, cluster2_ch, cluster3_ch]
time_clusters = [cluster1_time, cluster2_time, cluster3_time]

rhos = np.zeros((3, 3)) # (clusters, conditions)
pvals = np.zeros((3, 3))

for i in range(3):
    if i < 2:
        colors = {comparisons[0][j]: colors_list[0][j] for j in range(len(colors_list[0]))}
        evokeds = {cond: tasks[cond] for cond in comparisons[0]}
        times = evokeds[comparisons[0][0]][0].times
        erf_violins = {comparisons[0][k]: [] for k in range(len(comparisons[0]))}
    else:
        colors = {comparisons[1][j]: colors_list[1][j] for j in range(len(colors_list[1]))}
        evokeds = {cond: tasks[cond] for cond in comparisons[1]}
        times = evokeds[comparisons[1][0]][0].times
        erf_violins = {comparisons[1][k]: [] for k in range(len(comparisons[1]))}

    ch_inds = channel_clusters[i]
    time_inds = time_clusters[i]
    ...
    # Average the activity inside the shaded area for each subject
    if i < 2:
        for k, k_comp in enumerate(comparisons[0]):
            nsubj = len(evokeds[k_comp])
            for l in range(nsubj):
                erf_violins[k_comp].append(np.sqrt(np.mean( # rms
                    evokeds[k_comp][l].get_data()[np.ix_(picks, time_inds)] ** 2, axis=1))
        ...
        for k, k_comp in enumerate(comparisons[1]):
            nsubj = len(evokeds[k_comp])
            for l in range(nsubj):
                erf_violins[k_comp].append(np.sqrt(np.mean( # rms
                    evokeds[k_comp][l].get_data()[np.ix_(picks, time_inds)] ** 2, axis=1))
    ...
    for m, m_key in enumerate(erf_violins.keys()):
        corr_results = sp.stats.spearmanr(np.nanmean(np.array(erf_violins[m_key]), axis=0), np.nanmean(restmean[picks, :], axis=1))
        rhos[i, m] = corr_results.statistic
        pvals[i, m] = corr_results.pvalue
...
for i in range(3):
    ...
    for m, m_key in enumerate(erf_violins.keys()):
        ...
        p = pvals_correct[i, m]
        if i < 2:
            label = f"{comparisons_nicer3[0][m]} $ \\rho $={rho:.2f}{p2str(p)}"
        else:
            label = f"{comparisons_nicer3[1][m]} $ \\rho $={rho:.2f}{p2str(p)}"
        
        # label = f"{comparisons_nicer3[i][m]}"
        ax[i].scatter(np.nanmean(restmean[picks, :], axis=1), np.nanmean(np.array(erf_violins[m_key]), axis=0), c=colors[m_key], label=label)
        m, n = np.polyfit(np.nanmean(restmean[picks, :], axis=1), np.nanmean(np.array(erf_violins[m_key]), axis=0), 1)
        ax[i].plot(np.nanmean(restmean[picks, :], axis=1), m * np.nanmean(restmean[picks, :], axis=1) + n, c=colors[m_key])
        
        ax[i].legend(frameon=True, fancybox=False, loc="upper right")
        ax[i].spines[['right', 'top']].set_visible(False)
        ax[i].set_xlim((0.008, 0.02))
        # ax[i].set_xticks([0.008, 0.012, 0.016, 0.020])
        # ax[i].set_xlim((0.0, 0.02))
        ax[i].set_ylim((-10, 105))
        ...
        if i == 0:
            ax[i].set_ylabel("mERF (fT)")

        ax[i].set_xlabel("ACW (s)")

        ax[i].set_yticks(np.arange(20, 110, 20))
        ...
        ts_insets.append(inset_axes(ax[i], width="70%", height="20%", loc="lower right"))
        ...
        insets.append(inset_axes(ax[i], width="20%", height="20%", loc="lower left"))
...
f.savefig(pathjoin(figpath, "f5_acw_mERF_scatter_union.png"), dpi=300, transparent=True)
#####################################################################################################################
...
for i in range(len(Xs)):
    ...
    for i_clu, clu_idx in enumerate(good_cluster_inds):
        ...
        ax[row, col].spines[['bottom', 'left']].set_visible(True)
        ...
        if col == 0:
            ax[row,col].set_ylabel("mERF (fT)")
        if row == 1:
            ax[row,col].set_xlabel("ACW (s)")
...
plt.tight_layout()
fig.savefig(pathjoin(figpath, "f5_acw_mERF_scatter.png"), dpi=300, transparent=True)
